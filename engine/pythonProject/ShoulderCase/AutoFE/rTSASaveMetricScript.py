...
def rTSASaveMetricOnBoneImplantInterface(inputPath):
    myViewport = session.Viewport(name='Output model', origin=(10, 10), width=150, height=100)
    modelName = os.path.join(inputPath, "rTSA.odb")
    myOdb = session.openOdb(name=modelName)
    myViewport.setValues(displayedObject=myOdb)
    myViewport.makeCurrent()
    myViewport.maximize()
    leaf = dgo.LeafFromElementSets(elementSets=('SCAPULA#BONEIMPLANTINTERFACEELEMENTS',))
...
def rTSASaveMetricOnBoneScrewsInterface(inputPath):
    myViewport = session.Viewport(name='Output model', origin=(10, 10), width=150, height=100)
    modelName = os.path.join(inputPath, "rTSA.odb")
    myOdb = session.openOdb(name=modelName)
    myViewport.setValues(displayedObject=myOdb)
    myViewport.makeCurrent()
    myViewport.maximize()
    leaf = dgo.LeafFromElementSets(elementSets=('SCAPULA#BONESCREWSINTERFACEELEMENTS',))
    dg = session.DisplayGroup(leaf=leaf, name='BIInterface')
    dg1 = session.displayGroups['BIInterface']
    myViewport.odbDisplay.setPrimaryVariable(
        variableLabel='E',
    ...
    odb = session.odbs[modelName]
    session.writeFieldReport(
        fileName='EMaxPrincipalBoneScrewsInterface.csv',
        append=OFF,
        sortItem='Element Label',
        odb=odb,
        step=0,
    ...
    session.writeFieldReport(
        fileName='EVOLBoneScrewsInterface.csv',
        append=ON,
        sortItem='Element Label',
        odb=odb,
        step=0,
...
if __name__ == "__main__": #to have the possibility to call the file in function list with an argument
    path = str(sys.argv[-1])
    print(path)
    rTSASaveMetricOnBoneImplantInterface(path)
    rTSASaveMetricOnBoneScrewsInterface(path)